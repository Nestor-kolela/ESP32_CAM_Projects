...
void loop() {
  ...
  bError = client.sendBinary((const char*) fb->buf, fb->len);
  
  if(bError == false) Serial.println("Failed to send!");
  
  esp_camera_fb_return(fb);

  Serial.print(u32Counter++);
  Serial.println(" Packets sent!");

  CloseReason err;
  err = client.getCloseReason();

  if(err != CloseReason_None)
  {
    Serial.print("Connection closed ");
    Serial.println( static_cast <uint16_t> (err));
  }
}